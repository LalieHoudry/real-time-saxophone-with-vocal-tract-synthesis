...
tvec = (0:Nbuf)/Fs ;                    
params.NmodesSax = 2 ; params.NmodesCV = 1 ;                  % nmodes
...
zeta = 0.4 ;
% modal sax
load("modal_params_sax.mat",'Cks','sks','fvec','svec') ; 
params.Cks = Cks ; params.sks = sks ; 
fZmod = @(Zc,Ck,sk) Zc.*sum(Ck./(svec-sk) + conj(Ck)./(svec-conj(sk)),1);
Ck_sax = Cks(1:params.NmodesSax, 1) ; sk_sax = sks(1:params.NmodesSax, 1) ; 
Zmod_sax = fZmod(Zcs,Ck_sax,sk_sax) ;
% conduit vocal
Rm = 17.5e-3 ; params.Zms = params.R1^2/Rm^2 ;
w1 = 2*pi*200 ; xi1 = 0.5 ; A1 = 500 ; % params modaux du conduit vocal
fZmod_CV = @(Rm,An,wn,xin) rho*c/Rm^2.*sum(svec.*An./(svec.^2+wn.^2+2.*svec.*xin.*wn),1) ; 
Zmod_CV = fZmod_CV(Rm,A1,w1,xi1) ;
sk_CV = -xi1*w1 + 1i*w1*sqrt(1-xi1^2) ;
Ck_CV = (A1/2)*(1+1i*w1*xi1/(w1*sqrt(1-xi1^2))) ;
...
while 1
    ...
    if refreshBDb.Value
        ...
        BDax = uiaxes(controlfig,'Position',[margin/2 dl 6*dl 5*dl],'xlim',[0 2],'ylim',[1 1000]*2*pi,'zlim',[0 1],'CLim',[0.4*min(abs(sks(1,:)))/(2*pi) 1.1*max(abs(sks(params.NmodesSax,:)))/(2*pi)]) ; 
        BDax.XLabel.String = '$\gamma$'; BDax.XGrid = 'on'; % BDax.XTick = 0:0.1:gammalim(2); % BDax.XTickLabel(~~mod(10*BDax.XTick,5)) = {''};
        BDax.YLabel.String = '$\omega_{CV}$'; BDax.YGrid = 'on'; % BDax.YTick = 0:0.1:zetalim(2); % BDax.YTickLabel(~~mod(10*BDax.YTick,5)) = {''};
        BDax.Colormap = [0 0 0; hsv];
