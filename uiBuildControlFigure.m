...
plot(zmodax,fvec(fmin:fmax),abs(Zmod_sax(fmin:fmax)/Zcs),fvec(fmin:fmax),abs(Zmod_CV(fmin:fmax)/Zcs),LineWidth=2) ; legend(zmodax,{"$Z_s$", "$Z_m$"}, "Interpreter", "Latex",FontSize=14);
title(zmodax,"Imp\'edances d'entr\'ee (approx modale)", "Interpreter", "Latex") ; grid(zmodax,"on") ;
xlabel(zmodax,"Fr\'equence (Hz)","Interpreter","Latex",FontSize=14) ; ylabel(zmodax,"$|Z_{mod}|$","Interpreter","Latex",FontSize=14) ; % zmodax.XLabel.String = "fr\'equence (Hz)" ; zmodax.YLabel.String = "dB" ; 
...
f0slider = uislider(controlfig,'Limits',[imag(params.sks(1,1))/(2*pi) imag(params.sks(1,end))/(2*pi)],"Position",[dtiers+margin/2 (margin+3*dl)/2 5*dl dl],"MajorTicks",imag(params.sks(1,:))./(2*pi),'MajorTickLabels',notenames,"Value",imag(sk_sax(1))/(2*pi),'ValueChangedFcn',@(f0slider,event) changedf0(event,f0slider,params)) ; 
...
w1 = event.Value*2*pi ; 
sk = -2*xi1*w1 + 1i*w1*sqrt(1-xi1^2) ;
assignin('base','w1',w1);
